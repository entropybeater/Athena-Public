...
TRAUMA_KEYWORDS = [r'rape', r'assault', r'scared', r'hurt', r'cry', r'sad', r'depress', r'suicid', r'kill']
SEX_KEYWORDS = [r'sex', r'fuck', r'suck', r'horny', r'cum', r'dick', r'top', r'bottom', r'hard']
PSYCH_KEYWORDS = [
    r'fix', r'curious', r'know', r'scared', r'fear', r'control', 
    r'safe', r'boring', r'ordinary', r'game', r'test', r'experiment', 
    r'layer', r'mirror', r'mask', r'detach', r'analy', r'truth'
]
...
class ChatAnalyzer:
    ...
    def _get_text_messages(self) -> list:
        ...
        return [m for m in self.messages if m.get('type') == 'message' and isinstance(m.get('text'), str)]
...
class CostAnalyzer:
    ...
    def calculate_session_value(self, content: str, model: str = "opus", cache_rate: float = 0.90) -> dict:
        ...
        subs_in = (turns - 1) * ((cacheable / 1e6) * rates["cache_read"] + (fresh / 1e6) * rates["input"]) if turns > 1 else 0
        
        total_in = turn1_in + subs_in
        total_out = (turns * output_per_turn / 1e6) * rates["output"]
        
        return {
            "type": session_type, "total_value": turn1_in + subs_in + total_out,
            "checkpoints": checkpoint_count, "turns": turns
        }
    ...
    def analyze_all(self, model: str = "opus") -> dict:
        ...
        results = []
        for log in logs:
            if "Analysis" in log.name or "README" in log.name: continue
            try:
        ...
        return {"total": total, "weighted_avg": weighted_avg, "count": len(results), "active": len(active)}
...
"""
    ...
    chat_parser.add_argument("--mode", "-m", choices=["gemini", "keywords", "temporal"], 
                              default="gemini", help="Analysis mode")
